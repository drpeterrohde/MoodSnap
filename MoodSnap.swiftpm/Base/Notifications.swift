...
func updateNotifications(settings: SettingsStruct) {
    ...
    let times = [
        Calendar.current.dateComponents([.hour, .minute], from: settings.reminderTime[0]),
        Calendar.current.dateComponents([.hour, .minute], from: settings.reminderTime[1])
    ]
                       
    let triggers = [
        UNCalendarNotificationTrigger(dateMatching: times[0], repeats: true),
        UNCalendarNotificationTrigger(dateMatching: times[1], repeats: true)
    ]
